...
```bash
cd "<vault>/plugins/paper-translate"
bash scripts/install.sh --install-java
```

볼트 루트에서 실행할 때는 아래처럼 명시적으로 볼트 경로를 넘긴다.

```bash
bash plugins/paper-translate/scripts/install.sh --vault "<vault>"
```

이 설치 스크립트는 `/tmp` 아래 로컬 가상환경 생성, Python 의존성 설치, Java 확인, 레거시 `thesis/` 마이그레이션, `논문` 폴더 생성을 처리한다. 가상환경 경로를 바꾸고 싶으면 `PAPER_TRANSLATE_VENV=/custom/path` 환경변수를 사용할 수 있다.
...
4. 각 PDF마다:
   ...
   - 생성된 `논문/trn/{파일명}__source.md`와 manifest를 읽는다
   - 제목과 소제목은 영어 원문 병기 규칙으로 번역한다
   - 번역 노트를 `논문/trn/{파일명} 번역.md` 로 저장한다
   - `python3 plugins/paper-translate/scripts/trans.py finish --vault "<vault>" --manifest "<manifest>"` 로 완료된 PDF를 `논문/fin/` 으로 이동한다

`prepare`와 `finish`는 내부 단계다. 사용자가 `/trans`를 요청하면 Codex가 이 두 단계를 번역 작성 전후에 연결해서 처리한다.
